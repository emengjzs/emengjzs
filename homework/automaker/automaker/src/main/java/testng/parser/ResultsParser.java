...
public class ResultsParser {
    ...
    public TestNGResult parse(File[] f) {
        try {
            return parse(new FileInputStream(f[0]));
    ...
    public TestNGResult parse(InputStream paths) {
        if (null == paths) {
        ...
        try {
            bufferedInputStream = new BufferedInputStream(paths);
