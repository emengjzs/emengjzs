...
{	
	...
	int			BGM_Safe;				//查 T_BGMTBL._DT
	int			BGM_Danger;
	int			addrATBonus;   			//地址，小怪一般为A4，boss战为EC
...
typedef struct
{
	...
	ubyte		DUMMY_STRUCT_02[2];
	//ArrayBytes	temp(4);

	local int i = 0;
	for (i = 0; i < 8; i++)
		FileIndex	party1MSFileIndex <format=hex>;

	for (i = 0; i < 8; i++)
		FileIndex	party2MSFileIndex <format=hex>;	
	
	int			BGM;
	local string	display;
	SPrintf(display, "0x%03X", index);
} BattleSetAuto <read=readBattleSetAuto,write=writeBattleSetAuto>;

string readBattleSetAuto(BattleSetAuto &a)
{
	return	a.display;
}

void writeBattleSetAuto(BattleSetAuto &a, string s)
{
	// 复制用
}
...
typedef struct
{
	...
	// 大于0x2710的 自动战斗
	if (ReadUShort(addrT_btsetStart + 2) != 0x500 && ReadUShort(addrT_btsetStart + 4) == 0x500)
	{
		FSeek(addrT_btsetStart + 2);
		ushort			addrAddrBattle2710;
		addrAddrBattleEnd = ReadUShort(addrT_btsetStart + addrAddrBattle2710);
		
		if (addrAddrBattleEnd < addrAddrBattle2710)
		{
			addrAddrBattleEnd = addrAddrBattle2710 + 2;
		}
		FSeek(addrT_btsetStart + addrAddrBattle2710);
		while(1)
		{
			FSkip(2);
			if(ReadUShort(ReadUShort(FTell()) + addrT_btsetStart) == 0xFFFF)
		...
		local ushort	TOTAL2 = (addrAddrBattleEnd - addrAddrBattle2710) / 2;

		FSeek(addrT_btsetStart + addrAddrBattle2710);
		ushort			addr2[TOTAL2];//  <hidden=true>;
		
		for (i = 0; i < TOTAL2; i++)
		{

			FSeek(addr2[i] + addrT_btsetStart);	// 跳转
			BattleSetAuto   battleAuto;
		}
	}
